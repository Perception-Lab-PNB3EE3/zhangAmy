...
condition2_plot <- ggplot(df_condition2, aes(x=x, y=y)) +
  ...
  geom_smooth(method=lm , color="red", fill="#6699ff", se=TRUE) +
    stat_poly_eq(aes(label = ..eq.label..), 
    formula = y ~ x, 
    label.x = max(df_condition2$x) * 0.5,  # Position near the right
    label.y = max(df_condition2$y) * 0.9   # Position near the top
  )+
  theme_ipsum() +
   labs(
    title = "Condition 2",
...
condition3_plot <- ggplot(df_condition3, aes(x=x, y=y)) +
  ...
  geom_smooth(method=lm , color="red", fill="#6699ff", se=TRUE) +
    stat_poly_eq(aes(label = ..eq.label..), 
    formula = y ~ x, 
    label.x = max(df_condition3$x) * 0.5,  # Position near the right
    label.y = max(df_condition3$y) * 0.9   # Position near the top
  )+
  theme_ipsum() +
   labs(
    title = "Condition 3",
...
condition4_plot <- ggplot(df_condition4, aes(x=x, y=y)) +
  ...
  geom_smooth(method=lm , color="red", fill="#6699ff", se=TRUE) +
    stat_poly_eq(aes(label = ..eq.label..), 
    formula = y ~ x, 
    label.x = max(df_condition4$x) * 0.5,  # Position near the right
    label.y = max(df_condition4$y) * 0.9   # Position near the top
  )+
  theme_ipsum() +
   labs(
    title = "Condition 4",
